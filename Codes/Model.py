...
class UNET3D(tf.keras.Model):
    ...
    def conv3D(self,x,
               ...
               filter_size,
               activation = 'relu'):
        out = tf.keras.layers.Conv3D(filters,(filter_size,filter_size,filter_size),padding = "same")(x)
    ...
    def max_pool3D(self, x, filter_size,stride):
        out = tf.keras.layers.MaxPooling3D((filter_size,filter_size,filter_size),strides = stride)(x)
        return out
